...
import useruser.dao.ItemTitleDAO;
import useruser.dao.MOOCItemDAO;
import useruser.dao.MOOCRatingDAO;
import useruser.dao.MOOCUserDAO;
import useruser.dao.RatingFile;
import useruser.dao.TitleFile;
import useruser.dao.UserFile;

import com.google.common.collect.Maps;
import com.google.common.collect.Sets;

/**
 * Main class for running the user-user assignment. Each command line argument is a user:item pair to score.
 * <ul>
...
public class UUMain
{
	...
	public static Map<Long, String> predict(String[] args)
	{
		Map<Long, Set<Long>> toScore = parseArgs(args);
		...
		Map<Long, String> itemToPredictions = Maps.newHashMap();
		...
		for (Map.Entry<Long, Set<Long>> scoreRequest: toScore.entrySet())
		{
			...
			for (long item: items)
			{
				...
				String title = titleDAO.getItemTitle(item);
				itemToPredictions.put(item, score);

				System.out.format("%d,%d,%s,%s\n", user, item, score, title);
			}
		}

		return itemToPredictions;
	}
	 ...
	 */
	public static Map<Long, Set<Long>> parseArgs(String[] args)
	{
	...
	public static LenskitConfiguration configureRecommender()
	{
		...
		config.set(RatingFile.class).to(new File("useruserdata/ratings.csv"));

		// use custom item and user DAOs
		// our item DAO has title information
		config.bind(ItemDAO.class).to(MOOCItemDAO.class);
		// and title file
		config.set(TitleFile.class).to(new File("useruserdata/movie-titles.csv"));

		// our user DAO can look up by user name
		config.bind(UserDAO.class).to(MOOCUserDAO.class);
		config.set(UserFile.class).to(new File("useruserdata/users.csv"));
